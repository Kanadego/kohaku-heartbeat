...
| **v0.9 ✅** | 忙闲判定重做：键鼠退出繁忙判定 + 前台窗口类别裁决 + 焦点窗口实时探查 + 可见窗口枚举（2026-08-31 交付） | 复合判定，见 docs/busy-detection.md |
| v0.9.1 ✅ | 浏览流通道修复：Python urllib + 临时文件交接 + Bing 地域参数；DESIGN §6.5 功能实现手册（2026-08-31 推送） | 闲逛真实出素材 |
| **v0.10** | 自省愿望清单开发（2026-08-31 木偶人拍板提前）——**每周一次**自问三问题，只产出不自动改配置，防愿望自我污染；内容与节奏以木偶人拍板为准 | 从远期提前 |
| 远期 | 乙方案扫描补账（带脱敏管道） | 克制优先 |
